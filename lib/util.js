...
const util = {
  ...
  buildNode: (jobs = 0, options = config.defaultOptions) => {
    ...
    util.run('ninja', ['-C', config.outputDir, 'node'].concat(ninja_args), options)
    util.run('ninja', ['-C', config.outputDir, 'libuv'].concat(ninja_args), options)
    util.run('ninja', ['-C', config.outputDir, 'http_parser'].concat(ninja_args), options)
    util.run('ninja', ['-C', config.outputDir, 'cares'].concat(ninja_args), options)
    util.run('ninja', ['-C', config.outputDir, 'zlib'].concat(ninja_args), options)
  },

  buildMuon: (jobs = 0, options = config.defaultOptions) => {
    ...
    util.run('ninja', ['-C', config.outputDir, 'electron'].concat(ninja_args), options)
  },
